...
import { TeamAutoSummary } from "@/lib/analysis/buildTeamAutoSummary";
import { TeamTeleopSummary } from "@/lib/analysis/buildTeamTeleopSummary";
...
export default function TeamStrengthsWeaknessesCard({
    ...
    if (teleopSummary.averageDriverControl > 0 && teleopSummary.averageDriverControl < 3) {
        concerns.push("Driver control looks inconsistent");
    }
    if (teleopSummary.averageCycleSpeed > 0 && teleopSummary.averageCycleSpeed < 3) {
        concerns.push("Cycle speed may be slow");
    }
    if (teleopSummary.averageScoringEffectiveness > 0 && teleopSummary.averageScoringEffectiveness < 3) {
        concerns.push("Scoring effectiveness looks limited");
    }
    if (teleopSummary.averageDefenseResistance > 0 && teleopSummary.averageDefenseResistance < 3) {
        ...
        <Paper sx={{ p: 3 }}>
            <Stack spacing={2}>
                ...
                <Stack spacing={1}>
                    ...
                    {strengths.length > 0 ? (
                        strengths.map((item) => <Typography key={item}>• {item}</Typography>)
                    ) : (
